...
#define PlotterDebug false
#define ShouldCalMag false
...
#define Ki 0.0f
    ...
    void magcalMPU9250() {
        ...
        Serial.print(mag_bias[0], 1);
        Serial.println(" -> Mag X bias");
        Serial.print(mag_bias[1], 1);
        Serial.println(" -> Mag Y bias");
        Serial.print(mag_bias[2], 1);
        Serial.println(" -> Mag Z bias");

        float mx_g = (float) mag_bias[0] * mRes * magCalibration[0]; // save mag biases in G for main program
        float my_g = (float) mag_bias[1] * mRes * magCalibration[1];
        float mz_g = (float) mag_bias[2] * mRes * magCalibration[2];

        Serial.print(mx_g, 1);
        Serial.println(" -> Mag X G");
        Serial.print(my_g, 1);
        Serial.println(" -> Mag Y G");
        Serial.print(mz_g, 1);
        Serial.println(" -> Mag Z G");
    ...
    bool imu_loop() {
        ...
        if (readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01) {                              // On interrupt, check if data ready interrupt
            ...
            ax = (float) accelCount[0] *
                 aRes; // - accelBias[0];  // get actual g value, this depends on scale being set
            ...
            magbias[0] = -13.;//+470.; // User environmental x-axis correction in milliGauss, should be automatically calculated
            magbias[1] = 220.;//+120.; // User environmental x-axis correction in milliGauss
            magbias[2] = -112.;//+125.; // User environmental x-axis correction in milliGauss
